...
public class SearchSuggestHandler extends XmlHandler {
	...
	public ArrayList<ContentProviderOperation> parse(XmlPullParser parser,
		...
		batch.add(ContentProviderOperation.newDelete(SearchSuggest.CONTENT_URI)
				.build());
			...
			if (type == START_TAG) {
				...
				if (Tags.WORD.equals(tag)) {
					...
					batch.add(ContentProviderOperation
							.newInsert(SearchSuggest.CONTENT_URI)
							.withValue(SearchManager.SUGGEST_COLUMN_TEXT_1,
									text).build());
				}
			}
		}

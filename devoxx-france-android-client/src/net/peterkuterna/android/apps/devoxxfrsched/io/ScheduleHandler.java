...
public class ScheduleHandler extends JsonHandler {
	...
	protected static final String TAG = ScheduleHandler.class.getSimpleName();
	...
	private static String findOrCreateBlock(String kind, String code,
		...
		final String blockId = Blocks.generateBlockId(
				ParserUtils.isTalk(kind) ? type : kind, startTime, endTime);

		if (!isBlockExisting(blockId, resolver)) {
			final String title = ParserUtils.isTalk(kind) ? (ParserUtils
					.isKeynote(kind) ? kind : type) : kind;
